...
# Change History:
#
# Change 1 - On line 62 and 63 changed the $user variable to strip off the last 3 hex characters imposed by Exchange 2010 SP1 Rollup 6
...
function Find-X500Failures([int]$days=1,[string]$servers,[switch]$AutoHeal){
	...
	foreach ($usr in $UniqueUsers) {
		...
		if ($user) {
			#search for user
			$mbx=get-mailbox $user -ErrorAction 'SilentlyContinue' -ResultSize 1|where {$_.RecipientTypeDetails -ne "LegacyMailbox"}
			if ($mbx){
...
$mbx.EmailAddresses+=("X500:{0}" -f $usr)
						Set-Mailbox $mbx.identity -EmailAddresses $mbx.Emailaddresses
