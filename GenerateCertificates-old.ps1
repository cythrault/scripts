...
[System.Void](& certreq -new $CertRequestINF "$WorkDir\$Computer.req")
...
Write-Host "`nSubmitting request for $Computer to $CAConfig using $WorkDir\$Computer.req and writting to $WorkDir\$Computer.cer"
Remove-Item $WorkDir\$Computer.cer -ErrorAction SilentlyContinue
#[System.Void](& certreq -submit -config "$CAConfig" "$WorkDir\$Computer.req" "$WorkDir\$Computer.cer")
Write-Host "Accepting request for $WorkDir\$Computer.cer"
#[System.Void](& certreq -accept "$WorkDir\$Computer.cer")

Write-Host "Exporting certificate to $WorkDir\$PFXOutFile with private key."
[System.Void](& certutil -p $Password -exportPFX "$Computer.$DNSSuffix" "$WorkDir\$PFXOutFile")
...
if ($result -eq 0) {
	...
	[System.Void](& certutil –privatekey –delstore 'MY' "$Computer.$DNSSuffix")
}
