...
# Change History:
# Change 1 - On line 68 and 78 changed get and set commands to MailContact instead of Mailbox
...
function Find-X500Failures([int]$days=1,[string]$servers,[switch]$AutoHeal){
	...
	foreach ($usr in $UniqueUsers) {
		...
		if ($user) {
			#search for user
			$mbx=get-mailContact $user -ErrorAction 'SilentlyContinue' -ResultSize 1|where {$_.RecipientTypeDetails -ne "LegacyMailbox"}
			if ($mbx){
...
$mbx.EmailAddresses+=("X500:{0}" -f $usr)
						Set-MailContact $mbx.identity -EmailAddresses $mbx.Emailaddresses
